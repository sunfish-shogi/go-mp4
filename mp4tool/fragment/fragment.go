...
func (m *mp4fragment) buildFragments() error {
	for _, track := range m.trackMap {
		track.moofs = make([]moofInfo, 0)
	}

	sequenceNumber := uint32(1)
	for {
		next, err := m.buildFragment(m.videoTrackID, sequenceNumber)
		if err != nil {
			return err
		}
...
func (m *mp4fragment) buildFragment(trackID, sequenceNumber uint32) (bool, error) {
	track := m.trackMap[trackID]
	if track.lastSampleEnd >= len(track.samples) {
		return false, nil
	}

	sampleBegin := track.lastSampleEnd
	sampleEnd := track.lastSampleEnd
	...
	track.moofs = append(track.moofs, moofInfo{
...
func (m *mp4fragment) buildTfra(trackID uint32) error {
	// FIXME
	...
	tfra := &mp4.Tfra{
		...
		NumberOfEntry:         4,
		Entries:               make([]mp4.TfraEntry, 0, len(track.moofs)),
	}
	for _, moofInfo := range track.moofs {
